...
# Inicializar componentes avanzados (IL2.2, IL2.3, IL2.4)
...
async def chat(request: ChatRequest):
    """
    Chat directo con CookAI mejorado con validación de dominio, memoria persistente y contexto enriquecido.
    
    Cumple requisitos:
    - IL2.1: Herramientas de lectura, escritura y razonamiento
    - IL2.2: Memoria persistente entre sesiones
    - IL2.3: Planificación y decisiones multi-etapa
    - IL2.4: Recuperación semántica de contexto
    - RESTRICCIÓN: Solo responde sobre recetas
    ...
    try:
        ...
        user_id = request.mensaje[:10]  # Simplificado; en producción usar user_id real
        
        # 1. VALIDAR DOMINIO: Solo sobre recetas (Requisito especial del usuario)
        is_valid, error_msg = validate_and_filter_input(request.mensaje)
        if not is_valid:
            return {"respuesta": error_msg, "valido": False}
        
        # 2. EJECUTAR CON PLANIFICACIÓN Y CONTEXTO ENRIQUECIDO
        # Cumple IL2.2, IL2.3, IL2.4
        respuesta = execute_with_planning(request.mensaje, user_id=user_id)
...
async def recomendar(request: AgentRecommendRequest):
    """
    Endpoint mejorado del Agente CookAI con validación, planificación y memoria persistente.
    
    Cumple IL2.1, IL2.2, IL2.3, IL2.4 completamente:
    - Herramientas de lectura, escritura y razonamiento
    ...
    try:
        ...
        # 2. Construir pregunta clara
        user_id = "user_" + hashlib.md5(str(request.ingredientes).encode()).hexdigest()[:8]
        
        pregunta = f"Tengo estos ingredientes: {', '.join(request.ingredientes)}"
        if request.preferencia:
            pregunta += f" y prefiero {request.preferencia}"
        pregunta += ". ¿Qué receta me recomiendas?"
        
        # 3. VALIDAR DOMINIO
        is_valid, error_msg = validate_and_filter_input(pregunta)
        if not is_valid:
            return {"respuesta": error_msg, "valido": False}
        
        # 4. EJECUTAR CON SISTEMA COMPLETO
        respuesta = execute_with_planning(pregunta, user_id=user_id)
